...
class LocationCitiesAndDistrictsTableSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        foreach($cities AS $city) {
            $city_id = DB::table('location_cities')->insertGetId(['country_id'=>217,'name'=>$city['title']]);
            if(sizeof($city['districts'])) {
                foreach ($city['districts'] AS $district) {
                    DB::table('location_districts')->insert(['city_id'=>$city_id,'name'=>$district]);
                }
            }
        }
    }
